...
function severity(v, low, high) {
    const span = high - low;
    if (v < low)
        return Math.min(3, (low - v) / (span * 0.25));
    if (v > high)
        return Math.min(3, (v - high) / (span * 0.25));
    return 0;
}
Deno.serve(async (req) => {
    ...
    try {
        ...
        let risk_level;
        if (norm < 0.15 || abnormal_parameters.length === 0)
            risk_level = "Normal";
        else if (norm < 0.6 && abnormal_parameters.length <= 2)
