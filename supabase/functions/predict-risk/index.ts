// ML Risk Prediction — rule-based ensemble inspired by a Random Forest
...
// Reference ranges (adult). Derived from public CBC datasets.
const RANGES: Record<string, { low: number; high: number; unit: string; lowMsg: string; highMsg: string; weight: number }> = {
...
function classify(v: number, low: number, high: number): Status {
  if (v < low) return "low";
  if (v > high) return "high";
  return "normal";
}

// Severity: how far outside range (0 = normal, 1 = at boundary edge, >1 = far)
function severity(v: number, low: number, high: number): number {
  const span = high - low;
...
Deno.serve(async (req) => {
  ...
  try {
    const { values } = (await req.json()) as { values: Values };
    if (!values || typeof values !== "object") {
      return new Response(JSON.stringify({ error: "Invalid input: 'values' object required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const abnormal_parameters: string[] = [];
    ...
    // Normalize 0..1 then bucket
    const norm = weightSum > 0 ? riskScore / (weightSum * 1.5) : 0;
    let risk_level: "Normal" | "Mild Risk" | "High Risk";
...
function labelOf(k: string) {
  return ({ hemoglobin: "Hemoglobin", wbc: "WBC", rbc: "RBC", platelets: "Platelets", glucose: "Blood sugar" } as Record<string, string>)[k] ?? k;
}
