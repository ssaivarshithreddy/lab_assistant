...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isHF && isClinicalBert) {
      ...
      if (values.hemoglobin && values.hemoglobin.status === 'low') possible_conditions.push({ name: 'Possible anemia', confidence: 30, rationale: `Low hemoglobin (${values.hemoglobin.value} ${values.hemoglobin.unit}) - requires clinical confirmation` });
      if (values.wbc && values.wbc.status === 'high') possible_conditions.push({ name: 'Possible infection/inflammation', confidence: 30, rationale: `Elevated WBC (${values.wbc.value} ${values.wbc.unit}) - requires clinical confirmation` });

      const summaryParts = [];
      Object.entries(values).forEach(([k, v]) => { if (v) summaryParts.push(`${k.toUpperCase()}: ${v.value} ${v.unit} (${v.status})`); });
