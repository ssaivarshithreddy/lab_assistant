...
import { LogOut, Users, HardDrive, FileText, TrendingUp, Trash2, Eye, AlertTriangle, Loader2, RefreshCw } from "lucide-react";
        ...
        {loading ? (<div className="flex items-center justify-center h-96">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <PieChart>
                        <Pie data={pieData} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${value.toFixed(2)}MB`} outerRadius={80} fill="#8884d8" dataKey="value">
                ...
                <CardContent>
                  <ScrollArea className="w-full rounded-md border">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {userStats.map((user) => (<Fragment key={user.userId}>
                            <tr key={user.userId} className="border-b hover:bg-slate-50 cursor-pointer transition-colors duration-150" onClick={() => setExpandedUserId(expandedUserId === user.userId ? null : user.userId)}>
                              ...
                              <td className="px-4 py-3">
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                                    <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={(e) => e.stopPropagation()}>
                    ...
                    handleDeleteUser(user.userId, user.email);
                            ...
                            {expandedUserId === user.userId && (<tr className="bg-slate-50/50">
                                <td colSpan={6} className="px-6 py-4">
                                  <div className="border rounded-lg bg-white overflow-hidden shadow-sm">
                                    ...
                                    {reports.filter((r) => r.user_id === user.userId || (user.userId === 'unknown_user' && !r.user_id)).length === 0 ? (<p className="p-4 text-xs text-slate-500 text-center">No reports uploaded by this user.</p>) : (<div className="overflow-x-auto">
                            ...
                            .map((report) => (<tr key={report.id} className="border-b hover:bg-slate-50/30">
                                                  ...
                                                  <td className="px-4 py-2 text-right">
                                                    <div className="flex gap-2 justify-end">
                                                      <Button variant="ghost" size="sm" className="h-7 w-7 p-0 hover:bg-slate-100" onClick={(e) => {
                                e.stopPropagation();
                                ...
                                toast.success("Report ID copied");
                                                      ...
                                                      <AlertDialog>
                                                        <AlertDialogTrigger asChild>
                                                          <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-red-600 hover:text-red-700 hover:bg-red-50" onClick={(e) => e.stopPropagation()}>
            ...
            <TabsContent value="storage">
              <Card>
                ...
                const percentage = stats?.totalStorageMB && stats.totalStorageMB > 0
                    ? ((item.storageMB / stats.totalStorageMB) * 100).toFixed(2)
                    : "0.00";
                return (<tr key={item.userId} className="border-b hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  <div className="flex-1 bg-slate-200 rounded-full h-2">
                                    <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${Math.min(100, parseFloat(percentage))}%` }}/>
                                  </div>
            ...
            <TabsContent value="database">
              <Card>
                ...
                return (<tr key={report.id} className="border-b hover:bg-slate-50">
                                  ...
                                  <AlertDialog>
                                    ...
                                    <AlertDialogContent>
                                      ...
                                      <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction onClick={() => handleDeleteReport(report.id)} className="bg-red-600 hover:bg-red-700">
                                          Delete
                                        </AlertDialogAction>
                                      </AlertDialogFooter>
                                    </AlertDialogContent>
