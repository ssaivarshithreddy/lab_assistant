...
const Dashboard = () => {
    ...
    if (!current) {
        return (<Card className="mx-auto max-w-md p-8 text-center">
        ...
        try {
            if (isLocalReport(current) || !cloudEnabled()) {
                ...
                if (current.file_path) {
                    ...
                    if (storageError)
                        console.warn("Storage file delete failed; deleting report row anyway:", storageError);
                }
    ...
    return (<div className="space-y-6">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" disabled={deleting} onClick={() => void deleteCurrentReport()}>
                  {deleting ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Trash2 className="mr-2 h-4 w-4"/>}
                  Remove
                </AlertDialogAction>
              </AlertDialogFooter>
      ...
      {reports.length > 1 && (<div className="flex gap-2 overflow-x-auto pb-1">
          {reports.map((r) => (<Link key={r.id} to={`/dashboard/${r.id}`} className={cn("shrink-0 rounded-md border px-3 py-2 text-xs", r.id === current.id ? "border-primary bg-accent text-accent-foreground" : "border-border hover:bg-muted")}>
              {new Date(r.created_at).toLocaleDateString()}
            </Link>))}
        </div>)}

      {detectedMetricCount > 0 ? (<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {METRICS.map((m) => {
                const v = values[m.key];
                const status = v?.status ?? "normal";
                const Icon = status === "high" ? TrendingUp : status === "low" ? TrendingDown : CheckCircle2;
                return (<Card key={m.key} className="shadow-card">
        ...
        </div>) : (<Card className="shadow-card">
      ...
      <Card className="shadow-card">
        ...
        <CardContent>
          {current.prediction?.ai_engine ? (<div className="text-sm">
            ...
            </div>) : (<div className="text-sm text-muted-foreground">No AI engine metadata available for this report.</div>)}
        </CardContent>
      </Card>

      {current.prediction?.risk_level && (<Card className={cn("shadow-elevated border-2", current.prediction.risk_level === "Normal" && "border-success/40 bg-success-soft", current.prediction.risk_level === "Mild Risk" && "border-warning/50 bg-warning/10", current.prediction.risk_level === "High Risk" && "border-destructive/50 bg-destructive-soft")}>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center justify-between">
              ...
              <Badge className={cn("text-sm", current.prediction.risk_level === "Normal" && "bg-success text-success-foreground", current.prediction.risk_level === "Mild Risk" && "bg-warning text-warning-foreground", current.prediction.risk_level === "High Risk" && "bg-destructive text-destructive-foreground")}>
                {current.prediction.risk_level}
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {current.prediction.insights && current.prediction.insights.length > 0 ? (<ul className="space-y-1.5 text-sm">
                {current.prediction.insights.map((ins, i) => (<li key={i} className="flex items-start gap-2">
                    <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-foreground/60"/>
                    <span>{ins}</span>
                  </li>))}
              </ul>) : (<p className="text-sm text-muted-foreground">No abnormal patterns detected by the ML model.</p>)}
            {typeof current.prediction.confidence === "number" && (<p className="text-xs text-muted-foreground">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="shadow-card">
          ...
          <CardContent>
            {detectedMetricCount === 0 ? (<p className="text-sm text-muted-foreground">No standard lab risk indicators were detected. Review the AI summary for report findings.</p>) : abnormalCount === 0 ? (<p className="text-sm text-muted-foreground">All detected values are within normal range.</p>) : (<ul className="space-y-2 text-sm">
                {METRICS.filter((m) => ["low", "high"].includes(values[m.key]?.status ?? "")).map((m) => (<li key={m.key} className="flex items-center justify-between rounded-md bg-destructive-soft px-3 py-2">
                    <span>{m.label}</span>
      ...
      {trendData.length > 1 && (<Card className="shadow-card">
          <CardHeader>
