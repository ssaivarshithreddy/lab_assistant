import type { LabMetric, ParsedReport, MetricStatus } from "@/types/report";
...
function classify(value: number, low: number, high: number): MetricStatus {
...
function scoreConfidence(distance: number): number {
  return Math.max(0.55, Math.min(0.99, 0.92 - distance * 0.02));
}

function findMetricValue(text: string, aliases: string[]): { value: number; confidence: number } | null {
...
export function parseReportText(rawText: string): ParsedReport {
  ...
  const metrics: Record<string, LabMetric> = {};
