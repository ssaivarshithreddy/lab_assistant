...
import type { ValueObj } from "@/types/report";
...
export interface GroqStreamOptions {
  ...
  metricsContext: Record<string, ValueObj>;
 ...
 */
function buildSystemPrompt(metricsContext: Record<string, ValueObj>, reportSummary: string): string {
 ...
 */
export async function streamGroqChat(options: GroqStreamOptions): Promise<string> {
 ...
 */
export async function groqChat(options: Omit<GroqStreamOptions, "onChunk">): Promise<string> {
