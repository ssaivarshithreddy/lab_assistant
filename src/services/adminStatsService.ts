...
const fetchUserProfilesMap = async (): Promise<Map<string, UserProfileInfo>> => {
...
export const adminStatsService = {
  ...
  getSystemStats: async (): Promise<SystemStats | null> => {
    try {
      ...
      const userFirstReports = new Map<string, Date>();
      reports.forEach((report: any) => {
  ...
  subscribeToReports: (onDataChange: RealtimeCallback): (() => void) => {
  ...
  subscribeToProfiles: (onDataChange: RealtimeCallback): (() => void) => {
    const subscription = adminSupabase
